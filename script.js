...
function parseNALData(type, data) {
    ...
    return ensureAllFields(parsed, type);
}

function ensureAllFields(parsed, type) {
    const defaultFields = {
        vps: {
            vps_video_parameter_set_id: 0, vps_max_layers_minus1: 0, vps_max_sub_layers_minus1: 0, vps_temporal_id_nesting_flag: 0,
            vps_reserved_0xffff_16bits: 0, vps_sub_layer_ordering_info_present_flag: 0, vps_max_layer_id: 0, vps_num_layer_sets_minus1: 0,
            vps_timing_info_present_flag: 0, vps_extension_flag: 0
        },
        sps: {
            sps_seq_parameter_set_id: 0, chroma_format_idc: 1, separate_colour_plane_flag: 0, pic_width_in_luma_samples: 1920,
            pic_height_in_luma_samples: 1080, bit_depth_luma_minus8: 0, bit_depth_chroma_minus8: 0, log2_max_pic_order_cnt_lsb_minus4: 0,
            sps_max_dec_pic_buffering_minus1: 0, sps_max_num_reorder_pics: 0, sps_max_latency_increase_plus1: 0,
            restricted_ref_pic_lists_flag: 0, sps_extension_flag: 0
        },
        pps: {
            pps_pic_parameter_set_id: 0, pps_seq_parameter_set_id: 0, dependent_slice_segments_enabled_flag: 0,
            output_flag_present_flag: 0, num_extra_slice_header_bits: 0, pps_extension_flag: 0
        }
    };

    let requiredFields = {};
    if (type === 32) requiredFields = defaultFields.vps;
    if (type === 33) requiredFields = defaultFields.sps;
    if (type === 34) requiredFields = defaultFields.pps;

    return { ...requiredFields, ...parsed };
}
...
function parseVPS(data) {
    return {
        ...
        vps_temporal_id_nesting_flag: (data[2] >> 7) & 0x01,
...
function parseSPS(data) {
    return {
        ...
        chroma_format_idc: data[1] & 0x03,
        separate_colour_plane_flag: (data[1] >> 2) & 0x01,
        pic_width_in_luma_samples: ((data[3] & 0x0F) << 8) | data[4],
        pic_height_in_luma_samples: ((data[5] & 0x0F) << 8) | data[6],
